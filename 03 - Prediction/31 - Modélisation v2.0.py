...
def plot_para (df_result, country, date):
    ...
    for element in title:
        axs[k].plot(list_x, df_result.loc[:,element])
        axs[k].set_title(element)
        axs[k].grid()
        k+=1
    fig.suptitle(f'Parameters for {country} on\n{long_date}')
    
    logo = image.imread('https://github.com/CleHou/COVID19/raw/master/Logo2_200px.png')
    fig.figimage(logo, 30, 20, zorder=3)
    
    file_path = creation_folder ([f'/3.1 - Modelisation/{country}/{month}/Parameters'])
...
def modelisation (list_df, list_country):
    df_total_result = pandas.DataFrame(columns=['a_max', '% cas', 't_99', 'date_99', 'r^2', 'dC/dt_99'])
    for country in list_country:
        ...
        p_cas = data_to_fit[-1]/a_max
        
        dCdt_99 = reg_log_der (t_99, a_max,b_max,c_max)
        
        df_total_result.loc[country] = [numpy.int(a_max), p_cas, t_99, date_99, r2_max, dCdt_99]
...
def plot2 (a_df, country, list_date) :
    ...
    axs[0].plot(list_date, a_df.loc[:,'a (rmax)'],label='$r_{max}^2$', color=colors[0])
    axs[0].plot(list_date, a_df.loc[:,'a (amax)'],label='$a_{max}$', color=colors[1])
    axs[0].grid()
    axs[0].set_title(r'Variation de $a$ en fonction du temps')
    
    axs[1].plot(list_date, a_df.loc[:,'r (rmax)'],label='$r_{max}$', color=colors[0])
    axs[1].plot(list_date, a_df.loc[:,'r (amax)'],label='$a_{max}$', color=colors[1])
    axs[1].set_title(r'Variation de $r$ en fonction du temps')
    axs[1].grid()
    
    handles, labels = axs[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="center right", borderaxespad=0)
    plt.subplots_adjust(right=0.9)
    fig.suptitle(f"Determination du meilleur paramètre d'optimisation {country} \n {long_date}", size=16)
    fig.autofmt_xdate()
